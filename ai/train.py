...
def get_acc(predicted: torch.Tensor, truth: torch.Tensor, thresh: int = 60, is_combined: bool = False):
    ...
    return (dist.mean().item() + torch.all(predicted_keys == truth_keys, dim=1).float().mean().item()) / 2


def train_action_net(datasets: list[str], force_rebuild=False, checkpoint_model_id=None,
                     batch_size=64,
    ...
    train_set = OsuDataset(
        datasets=datasets, label_type=EModelType.Actions)
    ...
    try:
        ...
        if get_validated_input("Would you like to save the last epoch?\n", lambda a: True,
                               lambda a: a.strip().lower()).startswith("y"):
            model.save(project_name, datasets, best_epoch,
                       learning_rate, weights=best_state)
        return


def train_aim_net(datasets: list[str], force_rebuild=False, checkpoint_model_id=None, batch_size=64,
                  epochs=1, learning_rate=0.0001, project_name=""):
    if len(project_name.strip()) == 0:
        project_name = f"Project with {len(datasets)} Datasets"

    train_set = OsuDataset(
        datasets=datasets, label_type=EModelType.Aim)
...
def train_combined_net(datasets: list[str], force_rebuild=False, checkpoint_model_id=None, batch_size=64,
                       epochs=1, learning_rate=0.0001, project_name=""):
    if len(project_name.strip()) == 0:
        project_name = f"Project with {len(datasets)} Datasets"

    train_set = OsuDataset(
        datasets=datasets, label_type=EModelType.Combined)
...
def get_train_data(data_type: EModelType, datasets: list[int], datasets_prompt: str, models: list[dict], models_prompt: str):

    project_name = get_validated_input("What would you like to name this project ?",
                                 lambda a: True, lambda a: a.strip())
    ...
    return data_type,project_name, list(map(lambda a: datasets[a], selected_datasets)), checkpoint, epochs
...
"""
    ...
    for task in training_tasks:
        ...
        if task_type == EModelType.Aim:
            train_aim_net(
                project_name=project_name,datasets=dataset,checkpoint_model_id=checkpoint['id'] if checkpoint is not None else None, epochs=epochs)
        elif task_type == EModelType.Actions:
            train_action_net(project_name=project_name,datasets=dataset,checkpoint_model_id=checkpoint['id'] if checkpoint is not None else None, epochs=epochs)
        else:
            train_combined_net(project_name=project_name,datasets=dataset,checkpoint_model_id=checkpoint['id'] if checkpoint is not None else None, epochs=epochs)
